...
const MAX_WIDTH = 190;    // keeps the track from getting too sparse

// Fall back to a square if a logo is ever passed in without intrinsic dimensions.
function opticalSize(logo) {
...
export default function CompanyLogoCarousel({ logos, className = "" }) {
  ...
  const renderLogo = (company, key) => {
    const { width, height } = opticalSize(company.logo);

    return (
      <div
        key={key}
        className="w-[190px] h-[80px] flex-shrink-0 flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-300 opacity-70 hover:opacity-100"
      >
        <Image
          src={company.logo}
          alt={company.alt}
